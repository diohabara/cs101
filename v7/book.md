...
- **fd テーブル** — 開いているファイルディスクリプタの一覧（v6 で学んだ）
...
flowchart TD
    subgraph プロセス
        PID["PID: 1234"]
        MEM["メモリ空間"]
        FD["fd テーブル<br/>0: stdin<br/>1: stdout<br/>2: stderr"]
        REG["レジスタ状態"]
    end
```
...
```mermaid
flowchart TD
    P["親プロセス (PID=100)"] -->|fork()| C["子プロセス (PID=101)"]
    P -->|fork() 戻り値| PR["rax = 101 (子の PID)"]
    C -->|fork() 戻り値| CR["rax = 0"]
```
...
sequenceDiagram
    ...
    participant E as /bin/echo

    P->>C: fork() → 子を作成
    C->>C: execve("/bin/echo", argv, NULL)
    Note over C,E: プロセスイメージが<br/>置き換わる
    E->>E: "hello from execve" を出力
    E->>P: exit(0)
    P->>P: wait4() で回収
```
