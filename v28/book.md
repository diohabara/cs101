...
PS4 のハイパーバイザは、各ゲームプロセスに対して個別の VMCS を管理しています。VMCS のゲスト状態にゲームの CPU コンテキストを設定し、ホスト状態にハイパーバイザの復帰先を設定することで、ゲームとハイパーバイザ間の高速な切り替えを実現しています。v28 で構築した VMCS の設定パターンは、その基盤となる仕組みです。
