...
export const stageQuizzes: Record<string, StageQuiz> = {
  ...
  v2: {
    title: "理解確認クイズ",
    intro: "`mov` と比較命令の読み方を確認します。",
    questions: [
      {
        id: "mov-meaning",
        prompt: "`mov rax, rbx` の説明として正しいものはどれですか。",
        type: "single",
        options: [
          { id: "copy", label: "`rbx` の値を `rax` にコピーする", isCorrect: true },
          { id: "swap", label: "`rax` と `rbx` を同時に交換する", isCorrect: false },
          { id: "add", label: "`rax` に `rbx` を加算する", isCorrect: false },
        ],
        explanation: "`mov` はコピーです。元の `rax` の値は上書きされ、`rbx` は変わりません。",
      },
      {
        id: "compare-branch-points",
        prompt: "v2 の題材として観察している要素をすべて選んでください。",
        type: "multiple",
        options: [
          { id: "writeback", label: "どのレジスタへ結果が書き戻るか", isCorrect: true },
          { id: "cmp-result", label: "比較結果によって分岐条件がどう決まるか", isCorrect: true },
          { id: "paging", label: "ページテーブルの変換", isCorrect: false },
          { id: "vmexit", label: "VM exit の理由", isCorrect: false },
        ],
        explanation: "v2 は値の移動と比較の結果が後続の制御にどう影響するかを見る段階です。",
      },
      {
        id: "max-program",
        prompt: "`max.asm` を読むとき、特に追うべきものはどれですか。",
        type: "single",
        options: [
          { id: "comparison", label: "比較後にどちらの値が残るか", isCorrect: true },
          { id: "mmap", label: "どの仮想ページが割り当てられたか", isCorrect: false },
          { id: "signal", label: "どのシグナルで終了したか", isCorrect: false },
        ],
        explanation: "`max.asm` では比較のあとにどちらの値を保持するかが主眼です。",
      },
    ],
    resultMessages: {
      all_correct: "値の移動と比較の読み方は十分です。後続命令への影響まで追えています。",
      partially_correct: "大筋は良いです。`mov` がコピーである点と比較後の分岐条件を再確認してください。",
      needs_review: "v2 の要点がまだ混ざっています。`mov` と比較結果の意味に絞って読み直すと整理できます。",
    },
  },
  v3: {
    title: "理解確認クイズ",
