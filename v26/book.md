...
### プリエンプティブマルチタスク（Linux, Windows, PS4/PS5 で使用）
...
これらはすべて、Linux, Windows, PS4/PS5 の Orbis OS など、実際の OS カーネルが持つ基本機能と同じものです。

## PlayStation との関連

PS4/PS5 の Orbis OS カーネルは FreeBSD をベースにしたプリエンプティブマルチタスク OS です。ゲームプロセス、システムプロセス、GPU ドライバなどが並行して動作し、タイマー割り込みによる強制的なタスク切り替えが行われます。v26 で実装した協調的スケジューラは、プリエンプティブスケジューラの基本概念（タスクテーブル、状態管理、ラウンドロビン選択）を共有しています。
