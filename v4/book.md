...
![スタック構造](/images/v4/stack.drawio.svg)

> **Column: なぜ push すると「下に」伸びるのか**
>
> push/pop に本質的な上下はありません。「上に積む」はお皿を重ねる比喩で、LIFO の順序を伝えるためのものです。
>
> x86-64 のハードウェア実装では、スタックは高アドレスから低アドレスへ伸びます。初期のコンピュータでメモリが限られていた時代、コード・データは低アドレスから上へ、スタックは高アドレスから下へ伸ばすことで、同じ空間を最大限使える設計でした。この慣習が現在も続いています。
...
![ネストした call/ret のシーケンス図](/images/v4/call-ret-sequence.drawio.svg)
