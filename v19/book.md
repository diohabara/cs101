...
PS4/PS5 の Orbis OS カーネルも起動時に GDT を初期化します。ゲームプロセスは Ring 3（ユーザーモード）で実行され、カーネルは Ring 0 で動作します。GDT によるカーネル/ユーザー空間の分離は、ゲームプロセスがカーネルメモリに直接アクセスできないようにするセキュリティの基盤です。
