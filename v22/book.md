...
バディアロケータは Linux カーネルでも使われている物理メモリ管理手法です。メモリを $2^n$ サイズのブロックで管理します。

### オーダーとブロックサイズ

オーダー $n$ のブロックは $2^n$ ページ、すなわち $2^n \times 4\,\text{KB}$ です。
...
バディアドレスは $\text{addr} \oplus (1 \ll (\text{order} + 12))$ で計算できます:
...
XOR が使える理由: $2^n$ サイズでアラインされたブロックのペアは、サイズに対応するビットが 0/1 で交互になっています。
...
- `MAX_ORDER` $= 11$（Linux 6.x, $4\,\text{KB} \sim 8\,\text{MB}$）
