...
- **PCB**: Orbis OS（PS4）は FreeBSD ベースであり、FreeBSD の `struct proc` と `struct thread` がプロセス/スレッドの PCB に相当します。ゲームプロセスもこの構造体で管理されます
- **fork/exec**: PS4 のゲーム起動はシェルプロセスからの fork/exec に相当する機構で行われます。ゲームプロセスの PCB にはサンドボックス情報やメモリ予算などの PS4 固有フィールドが追加されています
- **状態遷移**: ゲーム中のサスペンド/レジューム機能は、プロセスの状態を RUNNING → BLOCKED に遷移させ、復帰時に BLOCKED → READY → RUNNING とする操作です
...
- [FreeBSD sys/proc.h](https://cgit.freebsd.org/src/tree/sys/sys/proc.h) — FreeBSD の `struct proc` 定義。Orbis OS の基盤
